...
<script>
	...
	import { collection, doc, setDoc, where, query, orderBy, onSnapshot } from 'firebase/firestore';
	...
	onMount(() => {
		if (browser) {
			...
			if (savedRecipe) {
				...
				recipes = JSON.parse(savedRecipe);
				recipeIngredients = recipes;
				let xx = recipeIngredients;
				console.log('recipe', xx);
			}
		}
	});
	...
	$effect(() => {
		...
		// check saved recipe in web storage
		...
		const unsubscribe = onSnapshot(
			...
			(querySnapshot) => {
				...
				ingredients = updatedItems; // Assign to $state variable
